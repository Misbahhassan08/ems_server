...
def energy_import_export_last_10_days(request):
    try:
        ...
        end_date = datetime.strptime("2025-06-13", "%Y-%m-%d").date()
        ...
        while current_date <= end_date:
            ...
            for analyzer_type, ep_label in [("Grid", "EP+"), ("Grid", "EP-"), ("Solar", "EP+"), ("Generator", "EP+")]:
                analyzers = Analyzer.objects.filter(gateway=gateway, type=analyzer_type)
                for analyzer in analyzers:
                    metadata_qs = MetaData.objects.filter(analyzer=analyzer, created_at__date=current_date)

                    for meta in metadata_qs:
                        for j in range(1, 21):
                            name = getattr(meta, f"value{j}_name", None)
                            value = getattr(meta, f"value{j}_value", None)
                            if name == ep_label:
                                try:
                                    float_val = float(value)
                                    key = f"{analyzer_type}_EP+" if ep_label == "EP+" else f"{analyzer_type}_EP-"
                                    if key in day_totals:
                                        day_totals[key] += float_val
                                except (TypeError, ValueError):
                                    continue

            data_by_day[day_str] = day_totals
...
@api_view(['GET'])
@permission_classes([permissions.AllowAny])
def sum_ep_plus_values(request):
    try:
        gateway_name = request.GET.get("gateway")
        if not gateway_name:
            return JsonResponse({"error": "Gateway name is required"}, status=400)
...
def Solar_import(request):
    try:
        ...
        for analyzer in analyzers:
            # Current value (latest EP+ from today)
            today_metadata = MetaData.objects.filter(
                analyzer=analyzer,
                created_at__date=today
            ).order_by('-created_at').first()

            # Previous value (latest EP+ from yesterday)
            yesterday_metadata = MetaData.objects.filter(
                analyzer=analyzer,
            ...
            if today_metadata:
                for i in range(1, 21):
                    if getattr(today_metadata, f"value{i}_name", None) == "EP+":
                        try:
                            current_total += float(getattr(today_metadata, f"value{i}_value", 0))
                        except (TypeError, ValueError):
                            pass

            if yesterday_metadata:
                for i in range(1, 21):
                    if getattr(yesterday_metadata, f"value{i}_name", None) == "EP+":
                        try:
                            previous_total += float(getattr(yesterday_metadata, f"value{i}_value", 0))
                        except (TypeError, ValueError):
                            pass
...
def Generator_import(request):
    try:
        ...
        # Filter analyzers by gateway and type 'Generator'
        analyzers = Analyzer.objects.filter(gateway=gateway, type='Generator')
...
def Grid_export(request):
    try:
        ...
        # For each analyzer, get latest and previous MetaData
        for analyzer in analyzers:
            metadata_qs = MetaData.objects.filter(analyzer=analyzer).order_by('-created_at')[:2]
...
@api_view(['GET'])
@permission_classes([permissions.AllowAny])
def Solar_Project(request):
    try:
        project_id = request.GET.get("Project_id")
        if not project_id:
            return JsonResponse({"error": "Project ID is required"}, status=400)

        # Step 1: Get all gateways with the same project ID
        ...
        # Step 2: Extract MAC addresses from gateways
        mac_addresses = gateways.values_list('mac_address', flat=True)

        # Step 3: Get all analyzers of type 'Solar' linked to these gateways
        analyzers = Analyzer.objects.filter(gateway__in=gateways, type='Solar')

        total_sum = 0

        # Step 4: For each analyzer, get latest metadata for matching MAC addresses
        for analyzer in analyzers:
            latest_metadata = MetaData.objects.filter(
                analyzer=analyzer,
            ...
            # Step 5: Extract EP+ value from value1_name to value20_name
            for i in range(1, 21):
                name = getattr(latest_metadata, f"value{i}_name", None)
                if name == "EP+":
                    value = getattr(latest_metadata, f"value{i}_value", None)
...
def Grid_Project(request):
    try:
        ...
        # Step 3: Get all analyzers of type 'Solar' linked to these gateways
        analyzers = Analyzer.objects.filter(gateway__in=gateways, type='Grid')
        ...
        return JsonResponse({"Total_Grid": total_sum}, status=200)
...
def Generator_project(request):
    try:
        ...
        # Step 3: Get all analyzers of type 'Solar' linked to these gateways
        analyzers = Analyzer.objects.filter(gateway__in=gateways, type='Generator')
        ...
        return JsonResponse({"Total_Generator": total_sum}, status=200)
...
@api_view(['GET'])
@permission_classes([permissions.AllowAny])
def Grid_export_project(request):
    try:
        project_id = request.GET.get("Project_id")
        if not project_id:
            return JsonResponse({"error": "Project ID is required"}, status=400)
...
def Grid_history(request):
    try:
        ...
        # For each analyzer, get the last 7 MetaData entries
        for analyzer in analyzers:
            metadata_entries = MetaData.objects.filter(analyzer=analyzer).order_by('-created_at')[:7]

            for metadata in metadata_entries:
                metadata_date = metadata.created_at.date()  # Extract date only
                ...
                for i in range(1, 21):
                    ...
                    if name == "EP+":
                        value = getattr(metadata, f"value{i}_value", None)
                        try:
                            daily_sums[metadata_date] += float(value)
                        except (TypeError, ValueError):
                            continue

        # Sort by date
        sorted_result = {
            str(date): daily_sums[date] for date in sorted(daily_sums.keys(), reverse=True)
        }

        return JsonResponse({"daily_EP+_sums": sorted_result}, status=200)
...
def Solar_history(request):
    try:
        ...
        return JsonResponse({"daily_Solar_sums": sorted_result}, status=200)
...
def Generator_history(request):
    try:
        ...
        return JsonResponse({"daily_Generator_sums": sorted_result}, status=200)
...
def selected_analyzer_data(request):
    try:
        ...
        for meta in metadata_qs:
            values = {}
            for i in range(1, 21):
                name = getattr(meta, f"value{i}_name", None)
                value = getattr(meta, f"value{i}_value", None)
                if name:
                    values[name] = value
